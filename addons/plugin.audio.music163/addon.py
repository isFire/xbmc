...
music_path = os.path.join(PROFILE, 'music')
video_path = os.path.join(PROFILE, 'video')
...
if 'MUSIC_U' not in account:
    account['MUSIC_U'] = ''
if '__csrf' not in account:
    account['__csrf'] = ''
if '__remember_me' not in account:
...
if not account['logined'] and xbmcplugin.getSetting(int(sys.argv[1]),'login') == 'true':
    ...
    if len(password)>0 and len(password)>0:
        ...
        account['logined'] = music.login_status()
        if account['logined']:
            account['uid'] = music.get_uid()
            dialog = xbmcgui.Dialog()            
            dialog.notification('登录成功', '请重启软件以解锁更多功能', xbmcgui.NOTIFICATION_INFO, 800,False)
        else:
            dialog = xbmcgui.Dialog()            
            dialog.notification('登录失败', '账号或密码错误', xbmcgui.NOTIFICATION_INFO, 800,False)
# logout            
if account['logined'] and xbmcplugin.getSetting(int(sys.argv[1]),'login') == 'false':   
    ...
    account['uid'] = ''
    dialog = xbmcgui.Dialog() 
    dialog.notification('退出成功', '账号退出成功', xbmcgui.NOTIFICATION_INFO, 800,False)
...
def get_songs(songs,privileges=[],picUrl=None,source=''):
    ...
    for i in range(len(songs)):
        ...
        if privilege is None:
            ...
            # if 'playMaxbr' in privilege:
            #     data['privilege']['playMaxbr'] = privilege['playMaxbr']
            # if 'pl' in privilege:
            #     data['privilege']['pl'] = privilege['pl']
            
            # if 'downloadMaxbr' in privilege:
            #     data['privilege']['downloadMaxbr'] = privilege['downloadMaxbr']
            # if 'dl' in privilege:
            #     data['privilege']['dl'] = privilege['dl']
            
            # if 'payed' in  privilege:
            #     data['privilege']['payed'] = privilege['payed']
            ...
            for words in song['lyrics']['range']:
                ...
                for index in range(len(index_list)):
                    ...
                    if index_list[index] >= second:
                        right = index
                        break
                temps.append({'first':first,'second':second,'left':left,'right':right})
            skip = []
...
def get_songs_items(datas, privileges=[],picUrl=None,offset=0, getmv=True,source='',sourceId=0):
    ...
    for play in songs:
        ...
        if mv_id>0 and xbmcplugin.getSetting(int(sys.argv[1]),'mvfirst')=='true' and getmv:
            context_menu = [
                ('播放歌曲','RunPlugin(%s)' % plugin.url_for('song_contextmenu',action='play_song',meida_type='song',song_id=str(play['id']),mv_id=str(mv_id),sourceId=str(sourceId),dt=str(play['dt']//1000))),
                ('收藏到歌单','RunPlugin(%s)' % plugin.url_for('song_contextmenu',action='sub_playlist',meida_type='song',song_id=str(play['id']),mv_id=str(mv_id),sourceId=str(sourceId),dt=str(play['dt']//1000))),
                ('收藏到视频歌单','RunPlugin(%s)' % plugin.url_for('song_contextmenu',action='sub_video_playlist',meida_type='song',song_id=str(play['id']),mv_id=str(mv_id),sourceId=str(sourceId),dt=str(play['dt']//1000))),
            ]
            items.append({
                'label': label,            
            ...
            context_menu = [
                ('收藏到歌单','RunPlugin(%s)' % plugin.url_for('song_contextmenu',action='sub_playlist',meida_type='song',song_id=str(play['id']),mv_id=str(mv_id),sourceId=str(sourceId),dt=str(play['dt']//1000))),
                ('分享歌曲','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='direct',id=0,title='分享'+ar_name+'的单曲',msg=play['name'],str_show='https://y.music.163.com/m/song/'+str(play['id'])+'/?userid='+str(account['uid']))),
                ('下载链接','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='song',id=play['id'],title='下载链接',msg=ar_name + ' - ' + play['name'],str_show=bitrate)),                
                ('歌曲ID:'+str(play['id']),''),
                # (play['name']+':'+str(play['id']),''),                
            ]

            if mv_id > 0:
                context_menu.append(('收藏到视频歌单','RunPlugin(%s)' % plugin.url_for('song_contextmenu',action='sub_video_playlist',meida_type='song',song_id=str(play['id']),mv_id=str(mv_id),sourceId=str(sourceId),dt=str(play['dt']//1000))))
                context_menu.append(('播放MV','RunPlugin(%s)' % plugin.url_for('song_contextmenu',action='play_mv',meida_type='song',song_id=str(play['id']),mv_id=str(mv_id),sourceId=str(sourceId),dt=str(play['dt']//1000))))                
...
@plugin.route('/show_qrcode/<type>/<id>/<title>/<msg>/<str_show>/')
def show_qrcode(type,id,title,msg,str_show):
    # if not xbmcvfs.exists(qrcode_path): # folder must end with slash or backslash
    if not os.path.exists(qrcode_path):
        SUCCESS = xbmcvfs.mkdir(qrcode_path)
        if not SUCCESS:
            dialog = xbmcgui.Dialog()
            dialog.notification('失败', '目录创建失败，无法使用该功能', xbmcgui.NOTIFICATION_INFO, 800,False)
            return
    ...
    if type == 'direct':
        ...
        songs = music.songs_url([id],bitrate=str_show)
        urls = [song['url'] for song in songs]
        url = urls[0]
        
        # xbmc.log('asdf - show - quality:%s,url:%s'%(quality,url))
        if url is None:  
            dialog = xbmcgui.Dialog()            
            dialog.notification('获取失败', '链接获取失败', xbmcgui.NOTIFICATION_INFO, 800,False)
            return
        else:
            str_show = url
    elif type == 'dj':
        result = music.dj_detail(id)
        song_id = result['program']['mainSong']['id']
        songs = music.songs_url([song_id],bitrate=bitrate)
        urls = [song['url'] for song in songs]
        url = urls[0]
        if url is None:  
            dialog = xbmcgui.Dialog()            
    ...
    img = qrcode.make(str_show)
    img.save(temp_path)
    dialog = xbmcgui.Dialog()
    dialog.notification(title, msg, temp_path, 5000,False)
    ...
    if action == 'sub_playlist':
        ...
        if ret == 0:
            ...
            if create_result['code'] == 200:
                ...
                dialog = xbmcgui.Dialog()            
                dialog.notification('创建失败', '歌单创建失败', xbmcgui.NOTIFICATION_INFO, 800,False)
        elif ret >= 1:            
        ...
        if ret >= 0:
            ...
            dialog = xbmcgui.Dialog()            
            dialog.notification('收藏', msg, xbmcgui.NOTIFICATION_INFO, 800,False)
    elif action == 'sub_video_playlist':
        ids = []
        names = []
        playlists = music.user_playlist(account['uid'],includeVideo=True)
        for playlist in playlists:            
        ...
        if url is None:
            dialog = xbmcgui.Dialog()            
            dialog.notification('播放', '该歌曲无法播放', xbmcgui.NOTIFICATION_INFO, 800,False)
        else:
            xbmc.executebuiltin('PlayMedia(%s)'%url)
    elif action == 'play_mv':
        mv = music.mv_url(mv_id)
        url = mv['url']
        if url is None:
            dialog = xbmcgui.Dialog()            
            dialog.notification('播放', '该视频已删除', xbmcgui.NOTIFICATION_INFO, 800,False)
        else:
            xbmc.executebuiltin('PlayMedia(%s)'%url)
...
def play(meida_type,song_id,mv_id,sourceId,dt):
    if meida_type == 'mv':
        ...
        if url is None:
            dialog = xbmcgui.Dialog()            
            dialog.notification('播放失败', '该视频已删除，自动播放歌曲', xbmcgui.NOTIFICATION_INFO, 800,False)
        ...
        if url is None:
            if int(mv_id)>0 and xbmcplugin.getSetting(int(sys.argv[1]),'auto_play_mv') == 'true':
                ...
                msg = '该歌曲无法播放'  
            dialog = xbmcgui.Dialog()            
            dialog.notification('播放失败', msg, xbmcgui.NOTIFICATION_INFO, 800,False)
        else:
...
def index():    
    ...
    items = [] 
    status = account['logined'] #and music.login_status()
    ...
    if xbmcplugin.getSetting(int(sys.argv[1]),'daily_recommend') == 'true' and status:
        items.append({'label':'每日推荐','path':plugin.url_for('recommend_songs')})  
    ...
    if xbmcplugin.getSetting(int(sys.argv[1]),'my_playlists') == 'true' and status:
        items.append({'label':'我的歌单','path':plugin.url_for('user_playlists',uid = account['uid'])})
    ...
    if xbmcplugin.getSetting(int(sys.argv[1]),'recommend_playlists') == 'true' and status:
        items.append({'label':'推荐歌单','path':plugin.url_for('recommend_playlists')})        
    ...
    if xbmcplugin.getSetting(int(sys.argv[1]),'top_mv') == 'true':
        items.append({'label':'热门MV','path':plugin.url_for('top_mvs',offset='0')})
    ...
    if xbmcplugin.getSetting(int(sys.argv[1]),'cloud_disk') == 'true' and status:
        items.append({'label':'我的云盘','path':plugin.url_for('cloud', offset = '0')}) 
    if xbmcplugin.getSetting(int(sys.argv[1]),'home_page') == 'true' and status:
        items.append({'label':'我的主页','path':plugin.url_for('user', id = account['uid'])})    
    if xbmcplugin.getSetting(int(sys.argv[1]),'new_albums') == 'true':        
        items.append({'label':'新碟上架','path':plugin.url_for('new_albums',offset='0')})
    if xbmcplugin.getSetting(int(sys.argv[1]),'new_albums') == 'true': 
        items.append({'label':'新歌速递','path':plugin.url_for('new_songs')})
    if xbmcplugin.getSetting(int(sys.argv[1]),'mlog') == 'true': 
        items.append({'label':'Mlog','path':plugin.url_for('mlog_category')})

    if xbmcplugin.getSetting(int(sys.argv[1]),'about') == 'true': 
        items.append({'label':'关于','path':plugin.url_for('about')})        
    return items    
...
def about(): 
    ...
    items.append({
        'label':'酷安@IECAS',
        'path':plugin.url_for('show_qrcode',type='direct',id=0,title='酷安',msg='来酷安关注我吧',str_show='http://www.coolapk.com/u/3189084'),
        'icon':os.path.join(BASE_RESOURCE_PATH, 'coolapk.png')
    })   

    items.append({
        'label':'网易云音乐@chen_1_0',
        'path':plugin.url_for('show_qrcode',type='direct',id=0,title='网易云音乐',msg='来网易云音乐关注我吧',str_show='http://music.163.com/m/user/home?id=347837981'),
        'icon':os.path.join(BASE_RESOURCE_PATH, 'netease.png'),
    })     
    items.append({
        'label':'当前版本 ' + str(ADDON.getAddonInfo('version')), 
        'path':plugin.url_for('show_qrcode',type='direct',id=0,title='酷安',msg='来酷安查看版本更新信息',str_show='http://www.coolapk.com/u/3189084'),
        'icon':None,
    })
    return items 
...
def mlog_category(): 
    # 1001视频,4演唱,2音乐安利,3演奏,1002MV,8001生活,6001舞蹈
    categories = {
        '视频':1001,
        '演唱':4,
        '音乐安利':2,
    ...
    for category in categories:
        if categories[category] == 1001:
            items.append({'label':category,'path':plugin.url_for('mlog',cid=categories[category],pagenum=1)})
        else:
            items.append({'label':category,'path':plugin.url_for('mlog',cid=categories[category],pagenum=0)})
    return items
...
def mlog(cid,pagenum):
    ...
    for video in mlogs:
        mid = video['id']
        if cid == '1002':
            path = plugin.url_for('play',meida_type='mv',song_id=0,mv_id=mid,sourceId=cid,dt=0)
            # mv = music.mv_url(mid)
            # url = mv['url']
        else:
            path = plugin.url_for('play',meida_type='mlog',song_id=0,mv_id=mid,sourceId=cid,dt=0)
            # detail = music.mlog_detail(mid)
            # url = detail['data']['resource']['content']['video']['urlInfo']['url']
    ...
    items.append({'label':tag('下一页','yellow'),'path':plugin.url_for('mlog',cid=cid,pagenum=int(pagenum)+1)})
    return items
...
def top_mvs(offset): 
    ...
    if more:
        items.append({'label':tag('下一页','yellow'),'path':plugin.url_for('top_mvs',offset=str(offset+limit))})
    return items

# 新歌速递
@plugin.route('/new_songs/')
def new_songs(): 
    songs = music.new_songs()
...
def new_albums(offset):    
    ...
    if len(albums)+ offset < total:
        items.append({'label':tag('下一页','yellow'),'path':plugin.url_for('new_albums',offset=str(offset+limit))})
    return items
    
# 排行榜
@plugin.route('/toplists/')
def toplists():
    items = get_playlists_items(music.toplists())
    return items

# 热门歌手
@plugin.route('/top_artists/')
def top_artists():
    return get_artists_items(music.top_artists())

# 每日推荐
@plugin.route('/recommend_songs/')
def recommend_songs():
    # return get_songs_items([song['id'] for song in music.recommend_playlist()])
    return get_songs_items(music.recommend_playlist())

# 历史日推
@plugin.route('/history_recommend_songs/<date>/')
def history_recommend_songs(date):
    # return get_songs_items([song['id'] for song in music.history_recommend_detail()])
    return get_songs_items(music.history_recommend_detail(date))
...
def get_albums_items(albums):
    ...
    for album in albums:
        ...
        if 'size' in album:
            plot_info = '[COLOR pink]' + name + '[/COLOR]  共' + str(album['size']) +'首歌\n' 
        else:
        ...
        if 'picUrl' in album:
            ...
            picUrl = album['cover']
        context_menu = [                
                ('分享专辑','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='direct',id=0,title='分享'+album['artists'][0]['name']+'的专辑',msg=name,str_show='http://music.163.com/album/'+str(album_id)+'/?userid='+str(account['uid']))),
            ]       
        items.append({
...
def albums(artist_id, offset):
    ...
    items = get_albums_items(albums)   
    if more:
        items.append({'label':tag('下一页','yellow'),'path':plugin.url_for('albums',artist_id=artist_id,offset=str(offset+limit))})
    return items            

@plugin.route('/album/<id>/')    
def album(id):
    # return get_songs_items([song['id'] for song in music.album(id)],sourceId=id)
...
def artist(id):
    items = [
        ...
        {'label':'所有歌曲', 'path':plugin.url_for('artist_songs', id = id, offset=0)},              
        {'label':'专辑', 'path':plugin.url_for('albums', artist_id=id,offset='0')},
        {'label':'MV', 'path':plugin.url_for('artist_mvs', id = id)},              
    ]
    
    info = music.artist_info(id)
    if 'accountId' in info:
        items.append({'label':'用户页', 'path':plugin.url_for('user',id=info['accountId'])})
    
    if account['logined'] and music.login_status():
        items.append({'label':'相似歌手', 'path':plugin.url_for('similar_artist',id=id)})
    return items

@plugin.route('/similar_artist/<id>/')
def similar_artist(id): 
    artists = music.similar_artist(id)
    return get_artists_items(artists)

@plugin.route('/artist_mvs/<id>/')
def artist_mvs(id): 
    mvs = music.artist_mvs(id)
    items = get_mvs_items(mvs)
    return items

@plugin.route('/hot_songs/<id>/')
def hot_songs(id):    
    result = music.artists(id)
...
def artist_songs(id,offset):    
    ...
    if result['more']:
        items.append({'label':'[COLOR yellow]下一页[/COLOR]','path':plugin.url_for('artist_songs',id=id,offset=int(offset)+limit)})
    return items
...
def song_purchased(offset): 
    ...
    if result.get('data',{}).get('hasMore',False):
        items.append({'label':'[COLOR yellow]下一页[/COLOR]','path':plugin.url_for('song_purchased',offset=int(offset)+limit)})
    return items
...
def dj_sublist(offset): 
    ...
    if result['hasMore']:
        items.append({'label':'[COLOR yellow]下一页[/COLOR]','path':plugin.url_for('dj_sublist',offset=int(offset)+limit)})
    return items

def get_djlists_items(playlists):
    items = []
    for playlist in playlists:
        plot_info = '[COLOR pink]' + playlist['name'] + '[/COLOR]  共' + str(playlist['programCount']) +'个声音\n'            
        if 'lastProgramCreateTime' in playlist and playlist['lastProgramCreateTime'] is not None:
            plot_info += '更新时间: ' + trans_time(playlist['lastProgramCreateTime']) + '\n'      
        ...
        if 'dj' in  playlist and playlist['dj'] is not None:
            plot_info += '创建用户: '+playlist['dj']['nickname'] + '  id: ' + str(playlist['dj']['userId']) + '\n'
        ...
        context_menu = [                
                ('分享播单','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='direct',id=0,title='分享'+playlist['dj']['nickname']+'的播单',msg=name,str_show='http://music.163.com/radio?id='+str(playlist['id'])+'&userid='+str(account['uid']))),
        ]  
...
def get_dj_items(songs,sourceId):
    ...
    for play in songs:   
        ...
        context_menu = [                
                ('分享声音','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='direct',id=0,title='分享'+play['radio']['name']+'的声音',msg=play['name'],str_show='http://music.163.com/program/'+str(play['id'])+'/'+str(play['dj']['userId'])+'/?userid='+str(account['uid']))),
                ('下载链接','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='dj',id=play['id'],title='下载链接',msg=play['radio']['name'] + ' - ' + play['name'],str_show='0')),
        ]
...
def digitalAlbum_purchased(): 
    # items = []
    albums = music.digitalAlbum_purchased()
...
def get_videos_items(videos):
    ...
    for video in videos:
        ...
        if type == 0:
            type = tag('[MV]')
            result =  music.mv_url(video['vid'])
            url = result['url']
        else:
            type = ''
            result =  music.video_url(video['vid'])
            url = result[0]['url']
        ar_name = '&'.join([str(creator['userName']) for creator in video['creator']])
        items.append({
...
def get_playlists_items(playlists):
    ...
    for playlist in playlists:
        context_menu = []
        plot_info = '[COLOR pink]' + playlist['name'] + '[/COLOR]  共' + str(playlist['trackCount']) +'首歌\n'            
        ...
        if 'subscribed' in playlist and playlist['subscribed'] is not None:
            if playlist['subscribed']:
                plot_info += '收藏状态: 已收藏\n'
                item = ('取消收藏','RunPlugin(%s)' % plugin.url_for('playlist_contextmenu',action='unsubscribe',id=playlist['id']))
                context_menu.append(item)
            else:
                if 'creator' in  playlist and playlist['creator'] is not None and str(playlist['creator']['userId'])!= account['uid']:
                    plot_info += '收藏状态: 未收藏\n'   
                    item = ('收藏','RunPlugin(%s)' % plugin.url_for('playlist_contextmenu',action='subscribe',id=playlist['id']))
                    context_menu.append(item)
        else:
            if 'creator' in  playlist and playlist['creator'] is not None and str(playlist['creator']['userId'])!= account['uid']:
                item = ('收藏','RunPlugin(%s)' % plugin.url_for('playlist_contextmenu',action='subscribe',id=playlist['id']))
        ...
        if 'creator' in  playlist and playlist['creator'] is not None:
            plot_info += '创建用户: '+playlist['creator']['nickname'] + '  id: ' + str(playlist['creator']['userId']) + '\n'
        ...
        if 'specialType' in playlist and playlist['specialType']==200:
            ...
            ptype = 'normal'
        if 'creator' in  playlist and playlist['creator'] is not None and str(playlist['creator']['userId'])== account['uid']:
            item = ('删除歌单','RunPlugin(%s)' % plugin.url_for('playlist_contextmenu',action='delete',id=playlist['id']))
            context_menu.append(item)
        item = ('分享歌单','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='direct',id=0,title='分享'+creator_name+'创建的歌单',msg=playlist['name'],str_show='http://music.163.com/playlist/'+str(playlist['id'])+'/'+str(creator_id)+'/?userid='+str(account['uid'])))
        context_menu.append(item)
...
def video_sublist(): 
    return get_videos_items(music.video_sublist())
    
@plugin.route('/album_sublist/')
def album_sublist(): 
    return get_albums_items(music.album_sublist())
...
def get_artists_items(artists):
    ...
    for artist in artists:
        ...
        context_menu = [                
                ('分享歌手','RunPlugin(%s)'% plugin.url_for('show_qrcode',type='direct',id=0,title='分享歌手',msg=artist['name'],str_show='http://music.163.com/artist?id='+str(artist['id'])+'&userid='+str(account['uid']))),
            ] 
        items.append({
            'label':name,
    ...
    for user in users:
        ...
        if 'followed' in user:
            if user['followed']==True:
                ...
                context_menu = [('取消关注','RunPlugin(%s)' % plugin.url_for('follow_user',type='0',id=user['userId']))]
            else:
                plot_info += '\n'
                context_menu = [('关注该用户','RunPlugin(%s)' % plugin.url_for('follow_user',type='1',id=user['userId']))]
        else:
            plot_info += '\n'
    ...
    if type == '1':
        ...
        if 'code' in result:
            if result['code'] == 200:         
                ...
                xbmcgui.Dialog().notification('关注用户', '您已关注过该用户', xbmcgui.NOTIFICATION_INFO, 800,False)
            elif result['code'] == 400:
                xbmcgui.Dialog().notification('关注用户', '不能关注自己', xbmcgui.NOTIFICATION_INFO, 800,False)
            elif 'mas' in result:
                xbmcgui.Dialog().notification('关注用户', result['msg'], xbmcgui.NOTIFICATION_INFO, 800,False)
    else:
        result = music.user_delfollow(id)
        if 'code' in result:
            if result['code'] == 200:         
                xbmcgui.Dialog().notification('取消关注用户', '取消关注成功', xbmcgui.NOTIFICATION_INFO, 800,False)
            elif result['code'] == 201:
                xbmcgui.Dialog().notification('取消关注用户', '您已不关注该用户了', xbmcgui.NOTIFICATION_INFO, 800,False)
            elif 'mas' in result:
                xbmcgui.Dialog().notification('取消关注用户', result['msg'], xbmcgui.NOTIFICATION_INFO, 800,False)    
@plugin.route('/user/<id>/')
def user(id): 
    items = [
        {'label':'歌单','path':plugin.url_for('user_playlists',uid = id)},
        {'label':'听歌排行','path':plugin.url_for('play_record',uid = id)},
        {'label':'关注列表','path':plugin.url_for('user_getfollows',uid = id,offset='0')},
        {'label':'粉丝列表','path':plugin.url_for('user_getfolloweds',uid = id,offset=0)},
    ]

    if account['uid'] == id:
        items.append({'label':'每日推荐','path':plugin.url_for('recommend_songs')}) 
        items.append({'label':'历史日推','path':plugin.url_for('history_recommend_dates')}) 
    
    info = music.user_detail(id)
    if 'artistId' in info.get('profile',{}):
        items.append({'label':'歌手页', 'path':plugin.url_for('artist',id=info['profile']['artistId'])})
    return items

@plugin.route('/history_recommend_dates/')
def history_recommend_dates():
    dates = music.history_recommend_recent()
    items = []
    for date in dates:
        items.append({'label':date,'path':plugin.url_for('history_recommend_songs',date=date)})
    return items

@plugin.route('/play_record/<uid>/')
def play_record(uid):
    items = [
        {'label':'最近一周','path':plugin.url_for('show_play_record',uid=uid,type='1')},
        {'label':'全部时间','path':plugin.url_for('show_play_record',uid=uid,type='0')},
    ]
    return items
...
def show_play_record(uid,type):  
    ...
    if code == -2:
        xbmcgui.Dialog().notification('无权访问', '由于对方设置，你无法查看TA的听歌排行', xbmcgui.NOTIFICATION_INFO, 800,False)
    elif code == 200:
        if type == '1':
...
def user_getfolloweds(uid,offset):
    ...
    if more:
        ...
        items.append({'label':tag('下一页','yellow'),'path':plugin.url_for('user_getfolloweds',uid=uid,offset=int(offset)+limit)})
    return items    
...
def user_getfollows(uid,offset): 
    ...
    if more:
        items.append({'label':'[COLOR yellow]下一页[/COLOR]','path':plugin.url_for('user_getfollows',uid=uid,offset=str(offset+limit))})
    return items

@plugin.route('/artist_sublist/')
def artist_sublist(): 
    return get_artists_items(music.artist_sublist())
...
def search():
    items = [
        ...
        {'label':'电台搜索','path':plugin.url_for('sea',type = '1009')},
    ]
    return items
...
def sea(type):
    ...
    if type=='-1':   
        ...
        if len(kws)==0:
            ...
            if result.get('hasMore',False):
                result = music.cloud_songlist(offset=2000,limit=result['count']-2000)
            ...
            for song in playlist:
                ...
                if 'ar' in song['simpleSong'] and song['simpleSong']['ar'] is not None and song['simpleSong']['ar'][0]['name'] is not None:
                    artist = " ".join([a["name"] for a in song['simpleSong']["ar"] if a["name"] is not None])
                else:
                    artist = song['artist']
                ...
                if 'alia' in song['simpleSong'] and song['simpleSong']['alia'] is not None:
                    alia = " ".join([a for a in song['simpleSong']["alia"] if a is not None])
                else:
                    alia = ''
        ...
        if len(datas)>0:
            ...
            dialog = xbmcgui.Dialog()            
            dialog.notification('搜索', '无搜索结果', xbmcgui.NOTIFICATION_INFO, 800,False)
            return 
    result = music.search(keyword,stype=type)
    # 搜索单曲
    if type=='1':        
        if 'songs' in result:
            ...
            if xbmcplugin.getSetting(int(sys.argv[1]),'hide_cover_songs') == 'true':
                filtered_songs = [song for song in sea_songs if '翻自' not in song['name'] and 'cover' not in song['name'].lower()]
            else:
    ...
    if type=='1006':        
        if 'songs' in result:
            ...
            items = get_songs_items(filtered_datas,privileges=filtered_privileges,source='search_lyric')
        else:
            dialog = xbmcgui.Dialog()            
            dialog.notification('搜索', '无搜索结果', xbmcgui.NOTIFICATION_INFO, 800,False)
            return            
    ...
    # 综合搜索
        ...
        if 'video' in result:
            ...
            for video in result['video']['videos']:
                if video['type'] == 0:                    
                    mv_url =  music.mv_url(video['vid'])
                    # xbmc.log('--test mv--%s'%mv_url)
                    url = mv_url['url']
                    ar_name = '&'.join([str(creator['userName']) for creator in video['creator']])
            ...
            for video in result['video']['videos']:
                if video['type'] == 1:                    
                    video_url =  music.video_url(video['vid'])
                    # xbmc.log('--test video--%s'%mv_url)
                    url = video_url[0]['url']
                    ar_name = '&'.join([str(creator['userName']) for creator in video['creator']])
...
def personal_fm():      
    # return get_songs_items([song['id'] for song in music.personal_fm()])
    songs = []
    for i in range(10):
        songs.extend(music.personal_fm())
    return get_songs_items(songs)

@plugin.route('/recommend_playlists/')
def recommend_playlists():
    return get_playlists_items(music.recommend_resource())

@plugin.route('/user_playlists/<uid>/')
def user_playlists(uid):
    return get_playlists_items(music.user_playlist(uid))
    ...
    if ptype == 'video':
        ...
        for data in datas:
            ...
            if 'song' in data['mlogExtVO']:
                artist = ", ".join([a["artistName"] for a in data['mlogExtVO']['song']['artists']])
                label += tag(' (' + artist + '-' + data['mlogExtVO']['song']['name'] + ')', 'gray') 
                context_menu = [
                    ('相关歌曲:%s'%(artist + '-' + data['mlogExtVO']['song']['name']),'RunPlugin(%s)' % plugin.url_for('song_contextmenu',action='play_song',meida_type='song',song_id=str(data['mlogExtVO']['song']['id']),mv_id=str(data['mlogBaseData']['id']),sourceId=str(id),dt=str(data['mlogExtVO']['song']['duration']//1000))),
                ]
            else:
                context_menu = []
...
def cloud(offset):    
    ...
    if more:
        items.append({'label':tag('下一页','yellow'),'path':plugin.url_for('cloud',offset=str(offset+limit))})
    return items
